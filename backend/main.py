...
FALLBACK_MODEL: str = os.environ.get("AZURE_OPENAI_FALLBACK_DEPLOYMENT", "gpt-5-nano")
...
async def generate_report(
    ...
    When ``?stream=1`` is supplied the response is delivered as an SSE
    stream of ``data: {"text": "<delta>"}`` events followed by a single
    ``data: {"final": "<full>"}`` event and a terminating ``data: [DONE]``
    line. Otherwise the legacy JSON shape ``{"report": "<full>"}`` is
    ...
    logger.info(
        "Report generation request received (visit_type=%s, stream=%s)",
        visit_type,
        bool(stream),
    )
    ...
    if MOCK_MODE:
        ...
        if stream:
            def _mock_stream():
                yield f"data: {json.dumps({'text': mock_report})}\n\n"
                yield f"data: {json.dumps({'final': mock_report})}\n\n"
                yield "data: [DONE]\n\n"
    ...
    if stream:
        # Stream raw deltas to the client for low TTFT, then emit a final
        # canonical full-text event so the client can swap to a single
        # authoritative string. Falls back to FALLBACK_MODEL on error
        # before any bytes have been sent.
        def _sse() -> "Generator[str, None, None]":
            ...
            for model in (CHAT_MODEL, FALLBACK_MODEL):
                ...
                try:
                    ...
                    yield f"data: {json.dumps({'final': full})}\n\n"
                    yield "data: [DONE]\n\n"
                    logger.info(
                        "Report streaming completed successfully (model=%s)", model
                    )
                    return
                except Exception as e:  # noqa: BLE001
                    last_err = e
                    logger.warning(
                        "Streaming model %s failed (started=%s): %s",
                        model,
                        started,
                        e,
                    )
                    if started:
                        # Already sent partial bytes — client will keep what
                        # it has. We cannot transparently retry mid-stream.
                        yield f"data: {json.dumps({'error': str(e)})}\n\n"
                        yield "data: [DONE]\n\n"
                        return
                    # Otherwise loop and try the fallback model.
            yield f"data: {json.dumps({'error': str(last_err) if last_err else 'unknown'})}\n\n"
            yield "data: [DONE]\n\n"
